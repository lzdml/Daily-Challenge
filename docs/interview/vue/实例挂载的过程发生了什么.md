...
**一. 分析**  
从入口代码开始分析, 先分析 `new Vue`背后发生了什么, 直接来看源码, 在 `src/core/instance/index.js`中.
...
首先可以看 `initMixin`方法, 发现该方法在`Vue`原型上定义了一个`_init`方法

源码位置: `src/core/instance/init.js`

::: details Vue.prototype.\_init
...
- 到了 `created`的时候, 数据已经初始化完成, 能够访问到 `data, props`这些属性, 但这时候并未完成`dom`的挂载, 因此无法获取到`dom`元素
...
- 在 挂载数据的时候, 不要将根元素放到 `body`或者`html`上
- 可以在对象中定义 `template/render`或者直接使用 `template, el`表示元素选择器
- 最终都会解析成`render`函数, 调用 `compileToFunctions`方法, 会将`template`解析成`render`函数

对`template`解析的步骤大致分为以下几步:

1. 将`html`文档片段解析成`ast`描述符
2. 将`ast`描述符解析成字符串
3. 生成`render`函数

生成`render`函数, 挂载到`vm`上后, 会再次调用`mount`方法

源码位置: `src/platform/web/runtime/index.js`

::: details mountComponent
...
render 的作用主要是生成 vnode, 源码如下: `src/core/instance/render.js`

::: details Vue.prototype.\_render
...
紧接着会调用 `vm._update(vnode, hydrating);`, \_update 主要作用是调用 `patch`,将 `vnode`转换为真实`DOM`,并更新到页面中
...
- 定义 `$on`, `$off`, `$emit`等事件
- 定义 `_update`, `$beforeUpdate`, `$destory`等生命周期
...
- `_update`将虚拟`DOM`生成真实`DOM`结构, 并渲染到页面中
